...
class SearchInstituteAdapter(var c: Context, var list: ArrayList<InstitudeModel>,val onClicked: (InstitudeModel) -> Unit) :
    RecyclerView.Adapter<RecyclerView.ViewHolder>(),Filterable{

    val searchableList = ArrayList<InstitudeModel>()
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        (holder as MyProjects).bind(searchableList[position].institutename.toString())
            ...
            override fun performFiltering(constraint: CharSequence?): FilterResults {
                ...
                else
                {
                    ...
                    for (item in 0..list.size) {
                        if (list[item].institutename!!.toLowerCase().contains(filterPattern)) {
